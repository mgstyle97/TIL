...
- 리스트는 배열과 연결 리스트를 이용하여 구현할 수 있다.
  ...
  ![](C:\Users\mgsty\download\git\TIL\Data structure\Linked_list\picture\list.PNG)
...
- 리스트의 pos 위치에 새로운 항목을 추가하려면 pos부터 마지막 항목까지 한 칸씩 오른쪽으로 이동하여 빈자리를 만든 후에, 새로운 항목을 pos 위치에 저장하여야 한다.
  ...
  ![](C:\Users\mgsty\download\git\TIL\Data structure\Linked_list\picture\operator_insert_array.PNG)
...
- pos 위치의 항목을 삭제하는 delete(list, pos) 구현
  ...
  ![](C:\Users\mgsty\download\git\TIL\Data structure\Linked_list\picture\operator_delete_array.PNG)
...
- **연결된 표현(linked representation)** : 동적으로 크기가 변할 수 있고 삭제나 삽입 시에 데이터를 이동할 필요가 없는 표현
  ...
  ![](C:\Users\mgsty\download\git\TIL\Data structure\Linked_list\picture\linked_representation.PNG)
...
- **연결 리스트(linked list)** : 연결된 표현으로 데이터들은 메인 메모리상의 어디에나 흩어져서 존재하고 물리적으로 흩어져 있는 자료들을 서로 연결하여 하나로 묶는 방법 
  ...
  ![](C:\Users\mgsty\download\git\TIL\Data structure\Linked_list\picture\operator_insert_linked_list.PNG)

  - 삭제 : 삭제 연산 또한 마찬가지로 데이터들을 옮길 필요가 없이 그냥 데이터들을 연결하는 줄만 수정하면 된다.

  ![](C:\Users\mgsty\download\git\TIL\Data structure\Linked_list\picture\operator_delete_linked_list.PNG)
...
- 연결 리스트의 그림에서 상자를 **노드(node)**라고 한다.
  ...
  ![](C:\Users\mgsty\download\git\TIL\Data structure\Linked_list\picture\linked_list_node.PNG)
...
- **이중 연결 리스트(doubly linked list)** : 하나의 링크는 앞에 있는 노드를 가리키고 또 하나의 링크는 뒤에 있는 노드를 가리킨다.
  ...
  ![](C:\Users\mgsty\download\git\TIL\Data structure\Linked_list\picture\linked_list_kinds.PNG)
...
- **단순 연결 리스트(singly linked list)**
  ...
  ![](C:\Users\mgsty\download\git\TIL\Data structure\Linked_list\picture\singly_linked_list.PNG)
...
- 노드는 자기 참조 구조체를 이용하여 정의된다.
  ...
  ![](C:\Users\mgsty\download\git\TIL\Data structure\Linked_list\picture\node.PNG)
...
- 일반적으로 연결 리스트에서는 필요할 때마다 동적 메모리 할당을 이용하여 노드를 동적으로 생성한다.
  ...
  ![](C:\Users\mgsty\download\git\TIL\Data structure\Linked_list\picture\create_node.PNG)

  ```C
  head = (ListNode *)malloc(sizeof(ListNode));
  ```

  ![](C:\Users\mgsty\download\git\TIL\Data structure\Linked_list\picture\reset_node.PNG)
...
- 단순 연결 리스트의 경우, 리스트의 처음이나 끝에 새로운 노드를 추가하는 경우가 많다.
  ...
  ![](C:\Users\mgsty\download\git\TIL\Data structure\Linked_list\picture\insert_first().PNG)
...
- insert()는 가장 일반적인 경우로서 연결 리스트의 중간에 새로운 노드를 추가한다.
  ...
  ![](C:\Users\mgsty\download\git\TIL\Data structure\Linked_list\picture\insert().PNG)
...
- 첫 번째 노드를 삭제하는 함수는 매개 변수로 head만 받으면 된다.
  ...
  ![](C:\Users\mgsty\download\git\TIL\Data structure\Linked_list\picture\delete_first().PNG)
...
- 연결 리스트를 변경할 head와 삭제할 노드의 전에 노드를 매개 변수로 받으면 된다.
  ...
  ![](C:\Users\mgsty\download\git\TIL\Data structure\Linked_list\picture\delete().PNG)
