...
- 변수는 중괄호 {} 블록 내에서 선언되고 사용된다.
  ...
  - 메소드 블록 내에서도 여러가지 중괄호 {} 블록들이 있을 수 있다.
...
- 자바는 모든 문자를 유니코드(Unicode)로 처리한다.
  ...
  - 유니코드는 0~66535 범위의 2byte 크기를 가진 정수값이다.
    - 0~127까지는 아스키(ASCII) 문자(특수기호 및 영어 알파벳)가 할당되어 있고, 44032에서 55203까지는 한글 11172자가 할당되어 있다.
...
- **short 타입** : 2byte(16bit)로 표현되는 정수값을 저장할 수 있는 데이터 타입이다.
  - 저장할 수 있는 값의 범위는 - 32,768에서 32,767(-2<sup>15</sup>~(2<sup>15</sup>-1)) 이다.
